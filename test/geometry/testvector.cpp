...
TEST(VectorTests, To1)
{
  ...
  //  EXPECT_EQ(v.to(FrameType::ECI).frame(), FrameType::ECI); // TODO: enable
  //  test
}

TEST(VectorTests, To2)
{
  //  Vector v(std::make_shared<Wgs84>(std::chrono::system_clock::now()), 80,
  //  80, 100); EXPECT_EQ(v.to(FrameType::ECI).frame(), FrameType::ECI); //
  //  TODO: enable test
}
...
{
  Vector a1(new ECI(nullptr));
  //  Vector a2(std::make_shared<Local>(std::shared_ptr<MockVector>()), 1, 2,
  //  3); EXPECT_FALSE(a1 == a2); EXPECT_TRUE(a1 != a2); // TODO: enable test
}
