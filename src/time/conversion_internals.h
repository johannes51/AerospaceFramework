...
typename std::enable_if_t<std::is_same_v<FromTime, ToTime>, ToTime>
...
template<typename ToTime, typename FromTime>
typename std::enable_if_t<std::is_base_of_v<CalendarTime, ToTime> & std::is_base_of_v<CalendarTime, FromTime> &
!std::is_same_v<FromTime, ToTime>, ToTime> convertInternal(const FromTime& from)
{
  return ToTime::unimplemented(from);
}

// General Julian to general Julian (same scale) -> ClassicalJulian to general Julian (same scale)
template<typename ToTime>
typename std::enable_if_t<std::is_base_of_v<JulianLikeTime, ToTime>, ToTime>
convertInternal(const ScaledJulianLike<typename ToTime::scale_type>& from)
{
  return convertInternal<ToTime>(from.toClassical());
}

// General Julian to general Julian (different scales) -> ClassicalJulian to general Julian (different scales)
template<typename ToTime, typename FromScale>
typename std::enable_if_t<std::is_base_of_v<JulianLikeTime, ToTime>, ToTime>
convertInternal(const ScaledJulianLike<FromScale>& from)
{
...
template<typename ToTime, typename FromScale>
typename std::enable_if_t<std::is_base_of_v<CalendarTime, ToTime>, ToTime>
convertInternal(const ScaledJulianLike<FromScale>& from)
{
...
template<typename ToTime>
typename std::enable_if_t<std::is_base_of_v<JulianLikeTime, ToTime>, ToTime>
convertInternal(const typename ToTime::scale_type& from)
{
  (void)from;
...
template<typename ToTime>
typename std::enable_if_t<is_instantiation_of_v<ToTime, ModifiedJulianDate>, ToTime>
convertInternal(const ClassicalJulianDate<typename ToTime::scale_type>& from)
{
  return ToTime(); // TODO: implement
}
...
template<>
TAI convertInternal<TAI, TT>(const TT& from);

template<>
TAI convertInternal<TAI, UT1>(const UT1& from);

template<>
TAI convertInternal<TAI, UTC>(const UTC& from);

template<>
TT convertInternal<TT, TAI>(const TAI& from);

template<>
TT convertInternal<TT, UT1>(const UT1& from);

template<>
TT convertInternal<TT, UTC>(const UTC& from);

template<>
UT1 convertInternal<UT1, TAI>(const TAI& from);

template<>
UT1 convertInternal<UT1, TT>(const TT& from);

template<>
UT1 convertInternal<UT1, UTC>(const UTC& from);
